...
|    [weather](https://github.com/nickustinov/weather-even-g2) | Weather forecast app @nickustinov/weather-even-g2 (git submodule) | ![weather](./media/weather.png) |
...
Initialize app submodules (required for `chess`, `epub`, `reddit`, `stars`, `transit`, `weather`):
...
npm --prefix apps/weather install
```
...
Note:
* `chess`, `epub`, `reddit`, `stars`, `transit`, and `weather` are sourced from upstream git submodules under `apps/*`.
...
3. Replace `even-dev/apps/quicktest/generated-ui.ts` with that generated code.
4. Run quicktest:
...
apps/weather    -> Upstream weather-even-g2 git submodule
